...
- **`outline-none` + `outline-3` draws no ring on the same element.** In Tailwind v4 `outline-none` sets `--tw-outline-style: none`, and the width utilities set `outline-style: var(--tw-outline-style)`, so a `focus-visible:outline-3` (or a parent's `[&_[data-highlighted]]:outline-3`) on an `outline-none` element gets a width and colour but style `none`. Add the matching `outline-solid` alongside it (`focus-visible:outline-solid focus-visible:outline-3 …`). A ring on a _different_ element from the `outline-none` one is fine.
- **bits-ui boolean data attributes are empty strings, not `"true"`.** `data-selected`, `data-disabled`, `data-highlighted` etc. are present as `""` when on, so style them with `data-selected:` / `data-[selected]:`, never `data-[selected=true]:` (matches nothing — the Command palette's highlight was invisible this way).
- **A Tailwind utility can't change `Layer`'s tint.** `Layer.svelte`'s `.tint` styles are unlayered component CSS, and unlayered CSS beats everything in `@layer utilities` whatever the specificity, so `[&[data-x]>.tint]:opacity-10` silently does nothing. For a keyboard/pointer highlight driven by an attribute, fill the item itself (`data-selected:bg-md-sys-color-on-surface/10`, as menus and `commandItem` do).
