  ...
  <Pane rounded={false}>
    <header class="mb-10 flex flex-col gap-3">
      ...
      <Body class="text-md-sys-color-on-surface-variant max-w-2xl">
        A single stateful button that switches between pressed and unpressed states — for standalone
        options like "show password" or a bold/italic formatting toggle, as opposed to a
    ...
    <section id="overview" class="mb-12 flex flex-col gap-4 scroll-mt-6">
      ...
      <Body>
        ...
        <code class="doc-code">usage: 'selection'</code>, giving it the same sizing and shape scale
        as <code class="doc-code">Button</code>, but with a binary pressed/unpressed visual state
        instead of variant/color styling.
      </Body>
      <Body>
