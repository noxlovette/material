<script lang="ts">
  ...
  const labelProps: PropRow[] = [
    ...
    },
    {
      ...
      default: '"medium"',
      desc: 'Selects the M3 type-scale token, e.g. md-sys-typescale-label-large.'
    },
    {
      prop: 'emphasized',
      type: 'boolean',
...
<svelte:head>
  <title>Label — Svelte Material Docs</title>
</svelte:head>
  ...
  <Pane rounded={false}>
    <header class="mb-10 flex flex-col gap-3">
      <div class="flex items-center gap-3">
        <nav class="text-md-sys-color-on-surface-variant text-sm" aria-label="Breadcrumb">
          <ol class="flex items-center gap-1">
            ...
            <li class="mx-1 opacity-40">/</li>
            <li class="text-md-sys-color-on-surface font-medium">Label</li>
          </ol>
        </nav>
        <span
      ...
      <Body class="text-md-sys-color-on-surface-variant max-w-2xl">
        <code class="doc-code">Label</code> is the smallest role in the type scale — used for
        component labels (buttons, tabs, chips) or very small text like captions. It renders a
        <code class="doc-code">&lt;p&gt;</code>.
      </Body>
    ...
    <section id="overview" class="mb-12 flex scroll-mt-6 flex-col gap-4">
      ...
      <Body>
        <code class="doc-code">Label</code> is one of five components — alongside
        <code class="doc-code">Display</code>, <code class="doc-code">Headline</code>,
        <code class="doc-code">Title</code>, and <code class="doc-code">Body</code> — implementing
        the
        <a
          href="https://m3.material.io/styles/typography/type-scale-tokens"
          target="_blank"
      ...
      <Body>
        ...
        typescale classes internally, so you'll typically reach for
        <code class="doc-code">Label</code> directly only for standalone captions or custom controls.
      </Body>
    </section>

    <section id="import" class="mb-12 flex scroll-mt-6 flex-col gap-4">
      <Title>Import</Title>
      <CodeBlock lang="typescript" code={`import { Label } from '@noxlovette/material';`} />
    </section>
    ...
    <section id="demo" class="mb-12 flex scroll-mt-6 flex-col gap-4">
      ...
      <a
        href={storybookStoryUrl('typography-label--playground')}
        target="_blank"
        rel="noopener noreferrer"
...
<Label size="small">Caption text</Label>`}
    ...
    <section id="label-props" class="mb-12 flex scroll-mt-6 flex-col gap-4">
    ...
    <section id="accessibility" class="mb-12 flex scroll-mt-6 flex-col gap-4">
      ...
      <div class="flex flex-col gap-3">
        {#each [{ icon: 'text_fields', title: 'Small text, real contrast', desc: 'At label sizes, contrast matters more, not less — always pair with an on-* color-role utility rather than a low-opacity color.' }, { icon: 'block', title: 'Not a heading', desc: 'Renders as a plain <p> — never use it as a substitute for a real heading level.' }] as item}
