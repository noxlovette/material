...
<svelte:head>
  <title>Popover — Svelte Material Docs</title>
</svelte:head>
  ...
  <Pane rounded={false}>
    ...
    <section id="overview" class="mb-12 flex scroll-mt-6 flex-col gap-4">
      ...
      <Body>
        ...
        <code class="doc-code">Popover</code> is this library's own general-purpose anchored-overlay
        primitive built on bits-ui's <code class="doc-code">Popover.Root</code>. It sits between a
        few M3 concepts without being a 1:1 match to any of them: closer to the non-modal "rich
        tooltip"/dialog surfaces described under
        <a
        ...
        >
        than to a true modal Dialog (it doesn't block interaction with the rest of the page and has no
        scrim), but it's also not a hover-triggered <code class="doc-code">Tooltip</code> (this library
        ships that separately under Forms) — it's click-triggered and can hold interactive content.
      </Body>
      <Body>
        Reach for <code class="doc-code">Popover</code> for click-triggered supplementary info or a
        small set of controls anchored to a button; reach for <code class="doc-code">Menu</code>
...
<Popover title="Storage">
  ...
  <Body>75% of your storage has been used.</Body>
    ...
    <section id="popover-props" class="mb-12 flex scroll-mt-6 flex-col gap-4">
