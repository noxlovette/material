...
<svelte:head>
  <title>Progress — Svelte Material Docs</title>
</svelte:head>
  ...
  <Pane rounded={false}>
    <header class="mb-10 flex flex-col gap-3">
      ...
      <Body class="text-md-sys-color-on-surface-variant max-w-2xl">
        ...
        <code class="doc-code">WavyLinearProgress</code> (a custom, M3-inspired wavy variant). Each has
        its own prop set — they are not interchangeable via a shared "variant" prop — so pick the component
        that matches the shape you need.
      </Body>
    ...
    <section id="overview" class="mb-12 flex flex-col gap-4 scroll-mt-6">
      ...
      <Body>
        ...
        for the underlying ARIA semantics, and share the same determinate/indeterminate convention: pass
        a <code class="doc-code">percent</code> between 0–100 for a determinate indicator, or omit
        it (or pass <code class="doc-code">null</code>) for an indeterminate one.
      </Body>
      <Body>
        ...
        >. <code class="doc-code">WavyLinearProgress</code> is this library's own extra variant — a
        sine-wave linear indicator inspired by, but not a literal reproduction of, M3's spec; it
