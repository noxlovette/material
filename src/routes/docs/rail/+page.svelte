...
<SupportingPane anchor="viewport" position="right" collapsible={false} rounded={false}>
  {#snippet main()}
    <SinglePane>
      <!-- ── Page header ─────────────────────────────────────────── -->
      ...
      <section id="overview" class="mb-12 flex flex-col gap-4 scroll-mt-6">
        ...
        <Body>
          ...
          <code class="doc-code">md</code> breakpoint it transparently becomes a bottom navigation
          bar (via its <code class="doc-code">withNavbar</code> prop).
        </Body>
        <Body>
          The Rail emits a <strong>ghost element</strong> — a hidden flex-item sibling that holds
          the same width as the rail. This means your page content is always pushed aside by the
          correct amount without any manual <code class="doc-code">margin-left</code> calculations.
        </Body>
      </section>
      ...
      <section id="import" class="mb-12 flex flex-col gap-4 scroll-mt-6">
        ...
        <CodeBlock
          lang="typescript"
          code={`import { Rail, RailItem } from '@noxlovette/material';`}
        />
      </section>
  ...
  <div class="ml-24">
    <!-- content -->
      ...
      <section id="responsive" class="mb-12 flex flex-col gap-4 scroll-mt-6">
        ...
        <Body class="text-md-sys-color-on-surface-variant">
          The Rail and its mobile fallback are designed to work together without any manual
          breakpoint checks in your code.
        </Body>
            ...
            <thead>
              <tr class="bg-md-sys-color-surface-container-highest">
                {#each ['Breakpoint', 'Rail', 'Scrim', 'Mobile navbar', 'Ghost width'] as h}
                  <th
                    class="border-md-sys-color-outline-variant border-b p-3 text-left font-semibold"
                    >{h}</th
                  >
                {/each}
              </tr>
      ...
      <!-- ── Page footer nav ────────────────────────────────────── -->
      <footer
        class="border-md-sys-color-outline-variant flex items-center justify-end border-t pt-8"
      >
        <a
          href="/docs/navbar"
          class="text-md-sys-color-primary flex items-center gap-1 text-sm no-underline hover:underline"
