  ...
  <Pane rounded={false}>
    <header class="mb-10 flex flex-col gap-3">
      ...
      <Body class="text-md-sys-color-on-surface-variant max-w-2xl">
        Cards group related content and actions about a single subject. This library ships all three
        M3 card types — elevated, filled, and outlined — as one component, switching between a <code
          class="doc-code">&lt;div&gt;</code
        >
        and an <code class="doc-code">&lt;a&gt;</code>
    ...
    <section id="overview" class="mb-12 flex flex-col gap-4 scroll-mt-6">
      ...
      <Body>
        ...
        >
        pattern. <code class="doc-code">type</code> selects the visual treatment (elevated / filled
        / outlined); everything else about the card's contents — title, media, actions — is left
        entirely to whatever you render inside <code class="doc-code">children</code>, matching M3's
        guidance that cards are a container, not a fixed template.
      </Body>
      <Body>
