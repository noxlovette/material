  ...
  <Pane rounded={false}>
    <header class="mb-10 flex flex-col gap-3">
      ...
      <Body class="text-md-sys-color-on-surface-variant max-w-2xl">
        ...
        >
        renders <code class="doc-code">position: absolute</code>, so it is an overlay primitive — it
        always needs a positioned (<code class="doc-code">relative</code>) ancestor to anchor
    ...
    <section id="overview" class="mb-12 flex flex-col gap-4 scroll-mt-6">
      ...
      <Body>
        ...
        positioned over their icon. Reach for <code class="doc-code">Badge</code> directly only when
        overlaying a count on something those components don't cover — e.g. a custom icon button —
        in which case wrap the anchor in a <code class="doc-code">relative</code> container yourself,
        as shown below.
      </Body>
    </section>
