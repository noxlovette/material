...
<svelte:head>
  <title>Button — Svelte Material Docs</title>
</svelte:head>
  ...
  <Pane rounded={false}>
    <header class="mb-10 flex flex-col gap-3">
      ...
      <Body class="text-md-sys-color-on-surface-variant max-w-2xl">
        ...
        comment. Five variants cover the full emphasis range, from <code class="doc-code"
          >elevated</code
        >
    ...
    <section id="overview" class="mb-12 flex scroll-mt-6 flex-col gap-4">
      ...
      <Body>
        Under the hood it renders Bits UI's <code class="doc-code">Button.Root</code>, wraps content
        with the shared <code class="doc-code">Layer</code> state-layer/ripple overlay, and supports
        a leading icon, a loading state, and rendering as an
