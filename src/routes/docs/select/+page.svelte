...
<svelte:head>
  <title>Select — Svelte Material Docs</title>
</svelte:head>
  ...
  <Pane rounded={false}>
    <header class="mb-10 flex flex-col gap-3">
      ...
      <Body class="text-md-sys-color-on-surface-variant max-w-2xl">
        ...
        trigger. It's powered by bits-ui's <code class="doc-code">Select.Root</code> and is entirely
        driven by an <code class="doc-code">options</code> array — it does not accept option elements
        as children.
      </Body>
...
<Select type="single" placeholder="Choose a fruit" {options} bind:value={fruit} />`}
    ...
    <section id="primitives" class="mb-12 flex scroll-mt-6 flex-col gap-4">
      ...
      <Body>
        ...
        prop, but <code class="doc-code">Select</code> itself does not expose a slot to compose them
        as children — its <code class="doc-code">Select.Content</code> is built entirely from
