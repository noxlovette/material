  ...
  <Pane rounded={false}>
    <header class="mb-10 flex flex-col gap-3">
      ...
      <Body class="text-md-sys-color-on-surface-variant max-w-2xl">
        ...
        imperative <code class="doc-code">toast()</code> function — it's a component you mount once
        (usually near the root of a page) with a
        <code class="doc-code">bind:message</code> prop, then trigger by assigning a string to that bound
        variable from anywhere in your app.
      </Body>
    ...
    <section id="overview" class="mb-12 flex flex-col gap-4 scroll-mt-6">
      ...
      <Body>
        ...
        >
        pattern. It watches its bindable <code class="doc-code">message</code> prop: when it becomes
        a non-empty string (or a Snippet), the snackbar slides in from the bottom; after 4 seconds
        it auto-dismisses and clears <code class="doc-code">message</code> back to an empty string
        (unless <code class="doc-code">static</code> is set, in which case it waits for the user to click
        the close button).
      </Body>
      <Body>
