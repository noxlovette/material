...
<script lang="ts">
  ...
  const itemClass = $derived(
    tv({
      base: `rounded-sm h-11 relative flex w-full cursor-pointer items-center gap-2 px-3 py-2 md-sys-typescale-body-medium whitespace-nowrap
      text-md-sys-color-on-surface hover:bg-md-sys-color-on-surface/8 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-md-sys-color-primary disabled:cursor-not-allowed disabled:opacity-38`,
      variants: {
        selected: {
          true: 'bg-md-sys-color-secondary-container rounded-lg text-md-sys-color-on-secondary-container'
        }
      }
    })({ selected })
  );
...
{#if !isGap}
  <DropdownMenu.Item {disabled} onSelect={handleSelect}>
    <button {...restProps} type="button" class={itemClass} {disabled}>
      {#if iconProps}
        <Icon class="text-md-sys-color-on-surface-variant size-5 text-[20px]" {...iconProps} />
      {/if}

      <div class="flex flex-1 flex-col gap-1 text-left">
        <span class="md-sys-typescale-body-medium text-md-sys-color-on-surface">
          {@render children()}
        </span>
        {#if helper}
