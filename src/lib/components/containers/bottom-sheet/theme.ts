...
export const bottomSheet = tv({
  slots: {
    base: [
      'fixed inset-x-0 top-auto bottom-0 z-40 mx-auto mt-auto mb-0 p-0',
    ...
    handleArea: 'flex h-12 shrink-0 cursor-grab touch-none justify-center active:cursor-grabbing',
    handle:
      'md-sys-state-focus-indicator flex h-12 w-12 items-center justify-center rounded-full outline-none',
    handleBar: 'bg-md-sys-color-on-surface-variant/40 h-1 w-8 rounded-full',
    body: 'min-h-0 flex-1 overflow-y-auto overscroll-contain pb-[env(safe-area-inset-bottom)]'
  },
  variants: {
    variant: {
      modal: {
